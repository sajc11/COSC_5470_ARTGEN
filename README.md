...
5. **Run the Jupyter Notebooks**
     ...
     ```
       jupyter notebook
     ```
     Open ```CNN_Training.ipynb``` or ```GAN_Training.ipynb``` to train the models or use ```GAN_Future_Implementations.ipynb``` for real-time art generation. Additionally, if your aim is to obtain more audio data, open the ```Spotify_Data.ipynb``` to obtain relevant metadata for chosen songs, or open ```Generate_Spectrograms.ipynb``` to translate the desired songs' mp3 to a spectrogram that can be used within the audio-to-art generation workflow.
